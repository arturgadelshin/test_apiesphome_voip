...
logging.basicConfig(
    level=logging.INFO,  # Держим INFO для нашего кода
...
global_sip_to_esp_queue = queue.Queue(maxsize=2000)
server_runner = None
...
class ESP32SIPAudioBridge:
    def __init__(self, esp_host, esp_port, esp_password, sip_target_uri, esp_clock_rate=16000):
        ...
        logger.info(f"🔧 ESP32SIPAudioBridge инициализирован")
        logger.info(f"  ESP32: {esp_host}:{esp_port}")
        logger.info(f"  SIP цель: {sip_target_uri}")
        ...
        async def stream_handler(request):
            ...
            try:
                while (self.stream_server_running and
                    ...
                    # Собираем до 1280 байт (40мс) или ждем 20мс
                    while total_bytes < 1280 and len(chunks) < 10:
                        try:
    ...
    async def make_call(self):
        ...
        try:
            ...
            if not call_answered:
                log_timing("⚠️ Звонок не ответили или не установлен")
                return False
    ...
    async def setup_audio_bridge_and_send_play(self, audio_media):
        ...
        try:
            ...
            # Запускаем стрим
            log_timing("🔄 Запуск HTTP-сервера стрима...")
            ...
            if self.cli and self.media_player_key:
                try:
                    ...
                    self.play_command_sent = True
                    log_timing("✅ Команда PLAY отправлена на ESP32.")
                except Exception as e:
                    log_timing(f"❌ Ошибка отправки PLAY: {e}")
            else:
                log_timing("❌ ESP32 клиент или медиаплеер не инициализированы")
            ...
            try:
                ...
                log_timing("🔄 Задача стрима завершена.")
            except asyncio.TimeoutError:
                log_timing("⏰ Таймаут ожидания завершения задачи стрима.")
                stream_task.cancel()
                try:
                    await stream_task
                except asyncio.CancelledError:
                    log_timing("🔄 Задача стрима отменена.")
            except Exception as e:
                log_timing(f"❌ Ошибка при ожидании завершения стрима: {e}")
    ...
    async def start_bridge(self):
        global BRIDGE_START_TIME, call_terminated
